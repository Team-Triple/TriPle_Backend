...
5. 사용자가 명시적으로 `작업 진행`이라고 승인하면 TASK별 DRAFT MD를 작성한다.
6. 사용자가 TASK 구현 파일과 DRAFT를 리뷰한다.
7. 사용자가 명시적으로 `병합!`이라고 승인하면 코드 반영을 시작한다.
8. 병합은 테스트 기반으로 진행한다.
9. 병합 후 커밋 전 사용자 이슈번호를 반드시 확인한다.
10. 테스트 결과와 병합/커밋/산출물 정리(PLAN/DRAFT 삭제) 내용을 보고한다.
...
- 해당 JOB의 마지막 TASK 병합 완료 시점까지 DRAFT 파일을 유지하고, 마지막 TASK 병합 완료 후 일괄 삭제한다. (무조건)
- 병합 후 커밋 전 사용자 이슈번호 확인 없이는 커밋하지 않는다.
- 해당 JOB의 마지막 TASK 커밋 완료 시 PLAN 파일을 즉시 삭제한다. (무조건)
