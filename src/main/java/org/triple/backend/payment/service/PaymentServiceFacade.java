...
public class PaymentServiceFacade {
    ...
    public PaymentConfirmRes confirm(PaymentConfirmReq paymentConfirmReq, Long invoiceId,
        Long userId) {
        Payment payment = paymentService.readyToInProgressPayment(paymentConfirmReq, invoiceId,
            userId);
        ...
        try{
            ...
            paymentService.failConfirm(paymentConfirmReq, PaymentStatus.EXPIRED);
            throw new BusinessException(PaymentErrorCode.CONFIRM_FAILED);
        } catch (ConfirmAnonymousException e) {
            log.error("알 수 없는 에러 발생", e);
            paymentService.failConfirm(paymentConfirmReq, PaymentStatus.ABORTED);
            throw new BusinessException(PaymentErrorCode.CONFIRM_FAILED);
