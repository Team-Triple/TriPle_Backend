...
@TestPropertySource(properties = {
        "app.search.group.fulltext-enabled=true",
...
class GroupServiceSearchFullTextMySqlTest {
    ...
    void FULLTEXT_검색은_PUBLIC_그룹_중_키워드와_매칭되는_그룹만_조회한다() {
        // given
        groupJpaRepository.save(Group.create(GroupKind.PUBLIC, "jeju-travelers", "weekend trip", "thumb", 10));
        groupJpaRepository.save(Group.create(GroupKind.PUBLIC, "busan-club", "jeju travel plan", "thumb", 10));
        groupJpaRepository.save(Group.create(GroupKind.PUBLIC, "seoul-walkers", "hangang walk", "thumb", 10));
        groupJpaRepository.save(Group.create(GroupKind.PRIVATE, "jeju-private", "jeju travel secret", "thumb", 10));
        ...
        assertThat(response.items())
                ...
                .containsExactlyInAnyOrder("jeju-travelers", "busan-club");
    ...
    void FULLTEXT_검색은_cursor_기반으로_다음_페이지를_중복_없이_조회한다() {
        // given
        for (int i = 1; i <= 12; i++) {
            groupJpaRepository.save(Group.create(GroupKind.PUBLIC, "jeju-travel-" + i, "trip plan", "thumb", 10));
        }
